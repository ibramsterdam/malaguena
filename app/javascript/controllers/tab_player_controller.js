...
export default class extends Controller {
  ...
  wrapSystem(lines, maxChars) {
    ...
    // At most four bars to a line; whatever doesn't fit — by count or by
    // width — moves to the next one.
    const maxBars = 4
    const groups = []
    let current = []
    ...
    bars.forEach(bar => {
      ...
      if (current.length > 0 && (current.length >= maxBars || currentWidth + barWidth > maxChars)) {
        groups.push(current)
        current = []
